...
filename_to_type(const char *FileName)
{
	...
	if (!dotpos) 
		return sf_unknown;
	size_t ext_length = len - (dotpos - FileName);
	if (ext_length==5 && !local_strcasecmp(dotpos, ".aiff"))
		return sf_aiff;
	if (len < 4)
        return sf_unknown;
    FileName += len - 4;
...
parse_args_(lame_global_flags * gfp, 
           const char *const inPath, const char *const outPath)
{
...
{
    int   ret;
    ret = parse_args_(gfp, inPath, outPath);
    return ret;
}
