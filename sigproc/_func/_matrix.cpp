...
void _buffer(CAstSig* past, const AstNode* pnode, const AstNode* p, string& fnsigs)
{
	...
	if (_blocklen != (double)(int)_blocklen)
		throw CAstException(FUNC_SYNTAX, *past, pnode).proc(fnsigs, "2nd argument must be an integer.");
	if (third.type() == TYPEBIT_NULL) _overlap = 0.;
	else
	{
		if (!third.IsScalar())
			throw CAstException(FUNC_SYNTAX, *past, pnode).proc(fnsigs, "3rd argument must be a scalar.");
		_overlap = third.value();
		if (_overlap != (double)(int)_overlap)
			throw CAstException(FUNC_SYNTAX, *past, pnode).proc(fnsigs, "3rd argument must be an integer.");
	}
...
void _ungroup(CAstSig* past, const AstNode* pnode, const AstNode* p, string& fnsigs)
{
	...
	if (past->Sig.IsAudio() || (type & 0x0003) <= 2) // audio, vector or constant
	{
		...
		int id = 0, id2 = 0;
		int nMoves = blocklen;
		for (unsigned int k = 0; k < past->Sig.nGroups; k++)
		{
			...
			//{
			//	past->Sig.buf[K * blocklen - overlap + m] += past->Sig.buf[K * blocklen + m];
			//}
